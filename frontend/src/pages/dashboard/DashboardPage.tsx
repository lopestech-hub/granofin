    ...
    <div className="rounded-xl bg-white border border-slate-200 shadow-sm p-5">
      <div className="flex items-start justify-between">
        <div>
          ...
          <p
            className={`mt-2 text-2xl font-bold font-mono tabular-nums tracking-tight ${
              tipo === 'saldo'
                ? valor >= 0
                  ? 'text-green-600'
                  : 'text-red-600'
          ...
          {quantidade !== undefined && (
            <p className="mt-1 text-xs text-slate-400">{quantidade} {quantidade === 1 ? 'lançamento' : 'lançamentos'}</p>
          )}
        </div>
        <div
    ...
    <div className="rounded-xl bg-white border border-slate-200 shadow-md p-3 min-w-[160px]">
      ...
      {payload.map((p: any) => (
        <div key={p.dataKey} className="flex items-center justify-between gap-6 text-xs">
          <span className="text-slate-600">{p.dataKey === 'receitas' ? 'Receitas' : p.dataKey === 'despesas' ? 'Despesas' : 'Saldo'}</span>
          <span
  ...
  return (
    <AppShell>
      <div className="p-6 max-w-7xl mx-auto space-y-6">
        ...
        {loadingResumo ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <CardMetrica
              ...
              tipo="receita"
              quantidade={resumo?.qtd_receitas}
            />
            <CardMetrica
              label="Despesas"
              valor={resumo?.total_despesas ?? 0}
              tipo="despesa"
              quantidade={resumo?.qtd_despesas}
            />
            <CardMetrica
              label="Saldo do mês"
        ...
        {evolucao.length > 0 && (
          <div className="rounded-xl bg-white border border-slate-200 shadow-sm p-5">
            <h2 className="text-sm font-semibold text-slate-900 mb-4">Evolução dos últimos 6 meses</h2>
        ...
        <div className="rounded-xl bg-white border border-slate-200 shadow-sm">
          <div className="flex items-center justify-between px-5 py-4 border-b border-slate-100">
            ...
            <a href="/lancamentos" className="text-xs text-green-600 hover:underline font-medium">
              Ver todos
            </a>
          </div>
                  ...
                  <div className="flex items-center gap-1 flex-shrink-0">
                    ...
                    <span
                      className={`font-mono tabular-nums text-sm font-semibold ${
                        l.tipo === 'RECEITA' ? 'text-green-600' : 'text-slate-800'
                      }`}
                    >
